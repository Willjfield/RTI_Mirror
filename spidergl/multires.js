...
var multiResRTI;
var openLink = false;
function createRtiViewer(idDiv, imageUrl, width, height, mirrorIdDiv,mirrorRTI)
{
	...
	var mirrorDiv = {};
	if (mirrorIdDiv != null){
		mirrorDiv.canvasNode = $($("#"+mirrorIdDiv)[0]);
		mirrorDiv.canvasName = mirrorIdDiv + "_webgl";
		mirrorDiv.canvas = $($(`#${mirrorIdDiv} canvas`)[0]);
		console.log(mirrorDiv)
	}else{
		mirrorDiv = null;
	}
	...
	$( "#"+idDiv + "_div #zoomIn" ).button({
	  ...
	  text: false,
      label: "Zoom In"
    }).on("pointerdown", function(e){multiResRTI.setRepeat(0); if(mirrorRTI)mirrorRTI.setRepeat(0); if (e.originalEvent.pointerType == "touch") touchEvent = true; else touchEvent = false;})
	.on("pointerup", function(e){multiResRTI.clearRepeat(e); if(mirrorRTI)mirrorRTI.clearRepeat(e); resetHoverState(0);})
	.on("pointerleave", function(e){multiResRTI.clearRepeat(e); if(mirrorRTI)mirrorRTI.clearRepeat(e); resetHoverState(0);})
	...
	$( "#"+idDiv + "_div #zoomOut" ).button({
	  ...
	  text: false,
      label: "Zoom Out"
    }).on("pointerdown", function(e){multiResRTI.setRepeat(1); if(mirrorRTI)mirrorRTI.setRepeat(1); if (e.originalEvent.pointerType == "touch") touchEvent = true; else touchEvent = false;})
	.on("pointerup", function(e){multiResRTI.clearRepeat(e); if(mirrorRTI)mirrorRTI.clearRepeat(e);  resetHoverState(1);})
	.on("pointerleave", function(e){multiResRTI.clearRepeat(e); if(mirrorRTI)mirrorRTI.clearRepeat(e); resetHoverState(1);})
	...
	$( "#"+idDiv + "_div #light" ).button({
	  ...
	  label: "Light Off"
		...
		if ( $( this ).text() == "Light Off" ) 
		{
			...
			if(mirrorRTI)mirrorRTI.setMode(1);
			...
			if(mirrorRTI)mirrorRTI.setMode(0);
		}
	...
	{
		divError.innerHTML = '';
		multiResRTI = new MultiRes(canvasName, mirrorRTI);
		// if(mirrorDiv != null){
		// 	mirrorDiv.rti = mirrorRTI;
		// }
...
function MultiRes(canvas, mirror)
{
	this.mode = 0;
...
MultiRes.prototype = {
	...
	// setMirror(_mirrorDiv){
	// 	this.mirrorDiv = _mirrorDiv;
	// },

	_setLightDir : function(x, y) 
	{
		if(this.mirror){ this.mirror._setLightDir(x, y); }
	...
	{
		if(this.mirror){_SGL_RegisteredCanvas[this.mirror.canvas].mouseDown(e);}
	...
	mouseDown : function(gl, button, x, y)
	{
		...
		if (button == 0)
		{
			...
			if(this.mirror){this.mirror.mouseDown(this.mirror.gl, this.mirror.button, x, y);}
	...
	{
		if(this.mirror){ this.mirror.mouseMove(this.mirror.gl, x, y); }
		if (this.animating)
			return false;
		if (this.ui.mouseButtonsDown[0])
		{
			if (this.mode == 0)
			{
				...
				//console.log(gl,x,y)//send event to other canvas here
